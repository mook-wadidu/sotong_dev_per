 ...
 */
export const DEMO_NARRATION: Partial<Record<string, string>> = {
  // 비트1 — 손님 화면(입력)
  lang: "손님은 앱 설치·가입 없이, 자기 핸드폰으로 텍스트를 쓰거나 이미지를 더해 상담지를 입력하고, 원하는 스타일·얼굴형·추구미 같은 메뉴를 선택해요.",
  // 비트2 — 디자이너가 한국어로 받음
  "d-inbox": "이렇게 손님이 완성한 상담지를, 디자이너는 한국어로 받아 확인해요.",
  // 비트3 — 양방향 실시간 번역 상담
  "d-chat": "손님이 외국어로 물어봐도 괜찮아요! 손님 말은 한국어로 번역돼 오고, 디자이너가 한국어로 답해도 손님 언어로 실시간 번역돼 상담이 이어져요.",
  // 비트4 — 시술지 완성·공유
  "d-record": "시술 전, 디자이너가 시술 내용을 시술지로 정리해 손님과 공유하고 시술을 진행해요.",
  // 비트5 — 리포트 + 별점 + 자산
  "d-report": "시술이 끝나면 전·후 사진을 포함한 리포트를 작성해 손님과 공유하고, 별점을 받아요. 이렇게 쌓인 리포트는 샵의 자산으로 남아요.",
};
