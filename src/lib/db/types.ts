...
export interface Repo {
  ...
  markDesignerViewed(designerToken: string): Promise<number>;
  /** 디자이너 "상담 시작" 시점 기록 — 손님 채팅 자동입장 신호(B1). */
  markChatStarted(designerToken: string): Promise<number>;
  /** 디자이너가 모발상태 입력을 시작한 시점 기록 — 손님 "입력 중" 신호(B2). */
  markDesignerEditing(designerToken: string): Promise<number>;
  /** 디자이너 "시술 시작" 1차 press — 동의 요청(plan_ready_at)(B동의). */
  requestConsent(designerToken: string): Promise<number>;
  /** 손님 [확인했어요] — customer_consented_at. consultationToken 키(B동의). */
  markConsented(consultationToken: string): Promise<number>;
  /**
