...
export interface IntakeDraft {
  ...
  consentedAt?: string;
  /**
   * **만 14세 이상 확인 시각 (ISO). 없으면 제출 차단.** PIPA §22-6 은 만 14세 미만 아동의
   * 개인정보를 법정대리인 동의 없이 처리하지 못하게 한다. 생년월일·나이를 저장하지 않고
   * 자기확인(attestation)만 기록해 최소수집 원칙과 양립시킨다.
   */
  age14ConfirmedAt?: string;
  /**
   * **민감정보(건강 — 알레르기) 처리 동의 시각 (ISO).** PIPA §23 은 민감정보를 다른 개인정보
   * 동의와 **별도로** 받도록 요구한다(일반 동의에 묶으면 위법). 이 값이 없으면 알레르기 관련
   * 항목(allergy/allergyNote)을 수집·저장하지 않는다 — 서버가 강제.
   */
  sensitiveConsentedAt?: string;
  /**
